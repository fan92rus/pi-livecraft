...
- Use a custom Pi build (fork): install the fork package globally (e.g. `npm i -g @fan92rus/pi-coding-agent@livecraft`) and Livecraft resolves it automatically from the installed `pi.cmd` shim. `PI_LIVECRAFT_PI_PACKAGE` is only needed when several Pi builds are installed and a specific one must be pinned (empty value falls back to shim discovery).
...
- Expose the UI to your local network: set `PI_LIVECRAFT_HOST=0.0.0.0` (Vite dev server) and `PI_LIVECRAFT_BACKEND_HOST=0.0.0.0` (HTTP API) before `npm run dev`. The manager channel always stays on `127.0.0.1`; only the web API and Vite bind to all interfaces. There is no authentication, so only do this on a trusted network.
